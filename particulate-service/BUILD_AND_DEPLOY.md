...
**Key advantages over PhysX-Anything:**
- **Much faster**: ~10 seconds vs 5-15 minutes per object
- **Smaller memory footprint**: 16GB vs 32GB RAM
- **Faster cold starts**: ~1-2 min vs 10-15 min
- **Better for internal parts**: Trained on geometry features, not VLM

**Use cases:**
- When you have a 3D mesh (from ZeroScene) and need articulation
...
**Note**: Build takes ~15-20 minutes (faster than PhysX-Anything).
...
- `--memory 16Gi`: Smaller than PhysX-Anything (uses less memory)
...
Or use the default (PhysX-Anything) for image-based inputs:

```bash
# Use PhysX-Anything (default)
ARTICULATION_BACKEND=physx

# PhysX service endpoint
PHYSX_ENDPOINT=https://physx-service-xxx.run.app
```

## Comparison: Particulate vs PhysX-Anything

| Feature | Particulate | PhysX-Anything |
|---------|-------------|----------------|
| **Input** | 3D mesh (GLB/OBJ) | Image (PNG/JPEG) |
| **Output** | Parts + URDF | Parts + URDF + Physics |
| **Speed** | ~10 seconds | 5-15 minutes |
| **Memory** | 16GB | 32GB |
| **Cold Start** | 1-2 minutes | 10-15 minutes |
| **Model Size** | ~100MB | ~10-20GB |
| **Strengths** | Internal parts, fast | Physical properties, in-the-wild |

### When to use Particulate:
- You have a 3D mesh from ZeroScene
- Fast turnaround is important
- Batch processing many objects
- Budget-conscious deployments

### When to use PhysX-Anything:
- Starting from an image (no mesh)
- Need physical properties (mass, friction, affordance)
- In-the-wild objects without good 3D reconstruction
...
**PhysX-Anything** (per 1000 objects):
- GPU time: ~170 hours (at ~10min each)
- Cost: ~$120 (L4 at $0.70/hr)

**Savings: ~98% cost reduction with Particulate!**
