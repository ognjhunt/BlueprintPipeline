...
class Paths:
    ...
    ASSET_INDEX = "geniesim/asset_index.json"
    TASK_CONFIG = "geniesim/task_config.json"
    GENIESIM_SCENE_CONFIG = "geniesim/scene_config.yaml"
    GENIESIM_EXPORT_MANIFEST = "geniesim/export_manifest.json"

    # Episodes (generated by Genie Sim or BlueprintPipeline)
    EPISODES_DIR = "episodes"
    EPISODES_META = "episodes/meta"
    EPISODES_DATA = "episodes/data"
    EPISODES_VIDEOS = "episodes/videos"
    EPISODES_INFO = "episodes/meta/info.json"
    EPISODES_MANIFEST = "episodes/meta/episodes.jsonl"
    TASKS_MANIFEST = "episodes/meta/tasks.jsonl"
...
class ScenePaths:
    ...
    asset_index: Path
    task_config: Path
    geniesim_scene_config: Path
    geniesim_export_manifest: Path

    # Episodes
    episodes_dir: Path
    episodes_meta: Path
    episodes_data: Path
    episodes_videos: Path
    episodes_info: Path
...
def get_scene_paths(root: Path, scene_id: str) -> ScenePaths:
    ...
    return ScenePaths(
        ...
        # Genie Sim 3.0
        geniesim_dir=scene_root / Paths.GENIESIM_DIR,
        scene_graph=scene_root / Paths.SCENE_GRAPH,
        asset_index=scene_root / Paths.ASSET_INDEX,
        task_config=scene_root / Paths.TASK_CONFIG,
        geniesim_scene_config=scene_root / Paths.GENIESIM_SCENE_CONFIG,
        geniesim_export_manifest=scene_root / Paths.GENIESIM_EXPORT_MANIFEST,
        # Episodes
        episodes_dir=scene_root / Paths.EPISODES_DIR,
        episodes_meta=scene_root / Paths.EPISODES_META,
        episodes_data=scene_root / Paths.EPISODES_DATA,
        episodes_videos=scene_root / Paths.EPISODES_VIDEOS,
        episodes_info=scene_root / Paths.EPISODES_INFO,
        episodes_manifest=scene_root / Paths.EPISODES_MANIFEST,
        tasks_manifest=scene_root / Paths.TASKS_MANIFEST,
    )
...
def get_geniesim_path(root: Path, scene_id: str) -> Path:
    """Get path for the Genie Sim directory."""
    return root / "scenes" / scene_id / Paths.GENIESIM_DIR


def get_episodes_path(root: Path, scene_id: str) -> Path:
    """Get path for the episodes directory."""
    return root / "scenes" / scene_id / Paths.EPISODES_DIR


def get_scene_graph_path(root: Path, scene_id: str) -> Path:
    """Get path for the Genie Sim scene graph."""
    return root / "scenes" / scene_id / Paths.SCENE_GRAPH


def get_asset_index_path(root: Path, scene_id: str) -> Path:
    """Get path for the Genie Sim asset index."""
    return root / "scenes" / scene_id / Paths.ASSET_INDEX
...
class StorageLayout:
    ...
    @property
    def geniesim_dir(self) -> Path:
        """Get Genie Sim directory path."""
        return self._paths.geniesim_dir

    @property
    def scene_graph(self) -> Path:
        """Get scene graph path."""
        return self._paths.scene_graph

    @property
    def asset_index(self) -> Path:
        """Get asset index path."""
        return self._paths.asset_index

    @property
    def episodes_dir(self) -> Path:
        """Get episodes directory path."""
        return self._paths.episodes_dir
    ...
    def ensure_directories(self) -> None:
        ...
        dirs = [
            ...
            self._paths.geniesim_dir,
            self._paths.episodes_dir,
            self._paths.episodes_meta,
            self._paths.episodes_data,
            self._paths.episodes_videos,
        ]
        for d in dirs:
