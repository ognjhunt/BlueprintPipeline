...
def patch_file():
    ...
    replacement = (
        ...
        f"{body_indent}# --- Camera diagnostics (log once) ---\n"
        f"{body_indent}if not getattr(self, '_bp_cam_diag_logged', False):\n"
        f"{body_indent}    self._bp_cam_diag_logged = True\n"
        f"{body_indent}    try:\n"
        f"{body_indent}        import omni.usd\n"
        f"{body_indent}        _diag_stage = omni.usd.get_context().get_stage()\n"
        f"{body_indent}        _diag_cam_prims = [str(p.GetPath()) for p in _diag_stage.Traverse() if p.IsA(omni.usd.UsdGeom.Camera) or 'Camera' in p.GetTypeName() or 'camera' in str(p.GetPath()).lower()]\n"
        f"{body_indent}        print(f'[CAMERA-DIAG] Camera-like prims on stage: {{_diag_cam_prims[:20]}}')\n"
        f"{body_indent}        _diag_cmd = getattr(self, 'data_received', {{}}) if hasattr(self, 'data_received') else {{}}\n"
        f"{body_indent}        print(f'[CAMERA-DIAG] Camera request data keys: {{list(_diag_cmd.keys()) if _diag_cmd else \"N/A\"}}')\n"
        f"{body_indent}        print(f'[CAMERA-DIAG] Camera prim from request: {{_diag_cmd.get(\"camera_prim\", \"NOT_SET\")}}')\n"
        f"{body_indent}    except Exception as _diag_err:\n"
        f"{body_indent}        print(f'[CAMERA-DIAG] Error during diagnostics: {{_diag_err}}')\n"
        f"{body_indent}try:\n"
        f"{indented_body}"
        ...
        f"{body_indent}        print(f'[PATCH] Camera prim requested: {{getattr(self, \"data_received\", {{}}).get(\"camera_prim\", \"UNKNOWN\")}}')\n"
        f"{body_indent}        print('[PATCH] Invalidating cached render product for retry on next call')\n"
        f"{body_indent}        # Invalidate cached render product so retry logic can recreate it\n"
        f"{body_indent}        _cls = type(self)\n"
        f"{body_indent}        for _rp_cache in ('_bp_render_products', '_bp_rgb_annotators', '_bp_depth_annotators'):\n"
        f"{body_indent}            _rp_dict = getattr(_cls, _rp_cache, {{}})\n"
        f"{body_indent}            if isinstance(_rp_dict, dict):\n"
        f"{body_indent}                _rp_dict.clear()\n"
        f"{body_indent}        _warmup = getattr(_cls, '_bp_warmup_done', set())\n"
        f"{body_indent}        if isinstance(_warmup, set):\n"
        f"{body_indent}            _warmup.clear()\n"
