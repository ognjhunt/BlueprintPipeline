...
GENIESIM_TRAJECTORY_FPS_ENV = "GENIESIM_TRAJECTORY_FPS"
...
DEFAULT_GENIESIM_GRPC_TIMEOUT_S = 60.0
...
DEFAULT_CIRCUIT_BREAKER_RECOVERY_TIMEOUT_S = 60.0
DEFAULT_CIRCUIT_BREAKER_BACKOFF_SECONDS = 2.0
DEFAULT_GENIESIM_TRAJECTORY_FPS = 15.0
...
DEFAULT_GENIESIM_OBJECT_POSE_MAX_CANDIDATES = 10
...
def get_geniesim_readiness_timeout_s(env: Optional[Mapping[str, str]] = None) -> float:
    """Return the configured Genie Sim readiness probe timeout in seconds."""
    source = env or os.environ
    return parse_float_env(
        source.get(GENIESIM_READINESS_TIMEOUT_S_ENV),
...
def get_geniesim_trajectory_fps(env: Optional[Mapping[str, str]] = None) -> float:
    """Return the configured trajectory FPS for gRPC execution (lower = fewer round-trips)."""
    source = env or os.environ
    return parse_float_env(
        source.get(GENIESIM_TRAJECTORY_FPS_ENV),
        default=DEFAULT_GENIESIM_TRAJECTORY_FPS,
        min_value=1.0,
        name=GENIESIM_TRAJECTORY_FPS_ENV,
    )
