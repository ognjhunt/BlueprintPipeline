#!/usr/bin/env python3
"""Index NVIDIA pack metadata and Stage 1 text generation assets into Firestore.
...
def index_stage1_assets(
    ...
    for obj in objects:
        ...
        doc = build_asset_document(entry, source="stage1")
        client.upsert_asset_document(doc)
        registered.append(doc.asset_id)
        print(f"[INDEXER] Registered Stage 1 asset {doc.asset_id} ({doc.class_name})")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Index pack metadata and Stage 1 assets")
    parser.add_argument("--pack-metadata", type=Path, help="Path to NVIDIA pack metadata JSON", required=False)
    parser.add_argument("--assets-prefix", type=str, help="Stage 1 assets prefix (e.g., scenes/123/assets)")
    parser.add_argument("--scene-id", type=str, default=None, help="Scene identifier for Stage 1 registration")
...
def main() -> int:
    ...
    if args.assets_prefix:
        registered.extend(index_stage1_assets(args.assets_prefix, args.scene_id, client, args.root))
