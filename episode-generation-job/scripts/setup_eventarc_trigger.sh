#!/bin/bash
# =============================================================================
# Setup EventArc trigger for Episode Generation Pipeline
# =============================================================================
#
# This script creates an EventArc trigger that invokes the episode-generation-pipeline
# workflow whenever a .usd_complete marker file is uploaded to GCS.
#
# The pipeline then runs Isaac Sim on GKE with GPU to generate physics-validated
# training episodes.
#
# Usage:
...
#   - episode-generation-pipeline workflow deployed
...
# Derived values
TRIGGER_NAME="episode-generation-trigger"
WORKFLOW_NAME="episode-generation-pipeline"
WORKFLOW_SA="workflow-invoker"

echo -e "${GREEN}=== Episode Generation EventArc Trigger Setup ===${NC}"
...
if ! gcloud iam service-accounts describe ${SA_EMAIL} --project=${PROJECT_ID} 2>/dev/null; then
    ...
    gcloud iam service-accounts create ${WORKFLOW_SA} \
        --display-name="Workflow Invoker SA for Episode Generation" \
        --project=${PROJECT_ID}
...
# Cloud Build invoker (to trigger builds that create GKE jobs)
gcloud projects add-iam-policy-binding ${PROJECT_ID} \
    --member="serviceAccount:${SA_EMAIL}" \
    --role="roles/cloudbuild.builds.editor" \
    --quiet

# GKE developer (to create jobs on cluster)
gcloud projects add-iam-policy-binding ${PROJECT_ID} \
    --member="serviceAccount:${SA_EMAIL}" \
    --role="roles/container.developer" \
    --quiet

# Storage access (read scenes, write episodes)
gcloud projects add-iam-policy-binding ${PROJECT_ID} \
    --member="serviceAccount:${SA_EMAIL}" \
...
echo ""
echo "Episode Generation Pipeline Configuration:"
echo "  Trigger: ${TRIGGER_NAME}"
echo "  Workflow: ${WORKFLOW_NAME}"
echo "  Bucket: ${BUCKET}"
echo "  Service Account: ${SA_EMAIL}"
echo ""
echo "The pipeline will trigger when .usd_complete markers are uploaded to:"
echo "  gs://${BUCKET}/scenes/{scene_id}/usd/.usd_complete"
echo ""
echo "This will run Isaac Sim on GKE to generate physics-validated episodes."
echo ""
echo -e "${YELLOW}Important: Ensure your GKE cluster has GPU nodes available!${NC}"
echo ""
