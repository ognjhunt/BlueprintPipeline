...
class LeRobotExporter:
    ...
    def _verify_parquet_exports(self, data_dir: Path) -> List[Tuple[Path, List[str]]]:
        ...
        try:
            ...
            for parquet_path in parquet_files:
                ...
                try:
                    if self._is_v3_export() and parquet_path.name.startswith("file-") and parquet_path.parent.name.startswith("chunk-") and parquet_path.parent.parent == data_dir:
                        rel_path = None
                        if self.output_dir:
                        ...
                        episodes_for_file = [
                            episode
                            for episode in self.episodes
                            if rel_path is not None and episode.parquet_path == rel_path
                        ]
                        if not episodes_for_file:
                            errors.append(
                                f"No episode metadata found for {parquet_path.name}"
                            )
                        if backend == "pyarrow":
                            ...
                            if episodes_for_file and num_row_groups != len(episodes_for_file):
                                errors.append(
                                    f"Row group count {num_row_groups} does not match expected "
                                    f"{len(episodes_for_file)} episodes"
                                )
                            for idx, episode in enumerate(episodes_for_file):
    ...
    def _write_episodes(self, data_dir: Path) -> None:
        ...
        try:
            ...
            for episode in self.episodes:
                ...
                if HAVE_PYARROW:
                    expected_rows = episode_data.num_rows

                    def _write_and_validate(tmp_path: Path, table=episode_data, n_rows=expected_rows) -> None:
                        pq.write_table(table, tmp_path)
                        self._validate_parquet_integrity(
                            tmp_path,
                            expected_num_rows=n_rows,
    ...
    def _write_episodes_v3(self, data_dir: Path) -> None:
        ...
        - meta/episodes/chunk-{idx:03d}/file-{idx:04d}.parquet (episode metadata)
        """
        if not HAVE_PYARROW:
            raise RuntimeError(
        ...
        for chunk_start in range(0, len(self.episodes), chunk_size):
            ...
            def write_parquet(tmp_path: Path, episodes=chunk_episodes) -> None:
                ...
                for episode in episodes:
                    ...
                    if self.config.include_images:
                        self._v3_episode_index[episode.episode_index]["video_paths"] = {
                            camera_id: self._get_v3_video_rel_path(
                                episode.episode_index,
                                camera_id,
                            )
                            for camera_id in self.config.camera_types
                        }
                    row_group_index += 1
                    expected_rows += episode_data.num_rows
                ...
                self._validate_parquet_integrity(
                    ...
                    expected_num_row_groups=len(episodes),
                )
    ...
    def _validate_parquet_integrity(
        ...
        expected_num_row_groups: Optional[int] = None,
    ) -> None:
        """Validate a Parquet file by reading it back after write.

        Checks:
        1. File is readable (valid footer metadata)
        2. Total row count matches expected
        3. Row group count matches expected (if provided)
        ...
        try:
            ...
            raise IOError(
                f"Parquet integrity check failed: file is not readable "
                f"({parquet_path.name}): {exc}"
            ) from exc

        metadata = parquet_file.metadata
        actual_rows = metadata.num_rows
        if actual_rows != expected_num_rows:
            raise ValueError(
                f"Parquet integrity check failed: expected {expected_num_rows} rows "
                f"but file contains {actual_rows} ({parquet_path.name})"
            )

        if expected_num_row_groups is not None:
            actual_groups = metadata.num_row_groups
            if actual_groups != expected_num_row_groups:
                raise ValueError(
                    f"Parquet integrity check failed: expected {expected_num_row_groups} "
                    f"row groups but file contains {actual_groups} ({parquet_path.name})"
                )
        ...
        if missing:
            raise ValueError(
                f"Parquet integrity check failed: missing columns {missing} "
                f"({parquet_path.name})"
            )

        self.log(
            f"  Parquet integrity verified: {actual_rows} rows, "
            f"{metadata.num_row_groups} row groups ({parquet_path.name})"
        )
    ...
    def _validate_lerobot_schema(
        ...
        expected_state_dim = self.robot_config.num_joints
        ...
        expected_action_dim = self.robot_config.num_joints + 1
    ...
    def _write_info(self, meta_dir: Path) -> None:
        ...
        if self._is_v3_export():
            ...
            videos_path_template = (
                "videos/{camera_id}/chunk-{chunk_idx:03d}/file-{file_idx:04d}.mp4"
                if self.config.include_images
                else None
            )
        else:
        ...
        def write_episodes(tmp_path: Path) -> None:
            with open(tmp_path, "w") as f:
                for episode in self.episodes:
                    ...
                    if self._is_v3_export() and self.config.include_images:
                        meta["video_paths"] = {
                            camera_id: self._get_v3_video_rel_path(
                                episode.episode_index,
                                camera_id,
    ...
    def _write_episode_index(self, meta_dir: Path) -> None:
        """Write v3 episode metadata as chunked Parquet in meta/episodes/.

        LeRobot v3.0 stores per-episode records (lengths, tasks, offsets) as
        chunked Parquet files at meta/episodes/chunk-{idx:03d}/file-{idx:04d}.parquet.
        """
        ...
        episodes_meta_root = meta_dir / "episodes"
        ...
        for chunk_start in range(0, len(self.episodes), chunk_size):
            ...
            episodes_meta_path = chunk_dir / f"file-{file_idx:04d}.parquet"

            chunk_episodes = self.episodes[chunk_start:chunk_start + chunk_size]
            episode_records = []
            for episode in chunk_episodes:
                ep_meta = self._v3_episode_index.get(episode.episode_index, {})
                video_paths = ep_meta.get("video_paths", {})
            ...
            def write_episodes_meta(tmp_path: Path, num_rows=num_episodes) -> None:
                ...
                if readback.metadata.num_rows != num_rows:
                    raise ValueError(
                        f"Episode metadata integrity check failed: expected {num_rows} "
    ...
    def _copy_video(
        ...
        dst_path = self._get_video_output_path(
            videos_dir,
            episode_index,
            camera_id,
        )
