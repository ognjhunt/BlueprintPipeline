...
JOB_NAME=${JOB_NAME:-"genie-sim-export-${SCENE_ID:-local}-$(date +%s)"}
...
printf "Deploying Genie Sim export job to GKE...\n"
printf "  Project:    %s\n" "${PROJECT_ID}"
printf "  Cluster:    %s\n" "${GKE_CLUSTER}"
printf "  Zone:       %s\n" "${ZONE}"
printf "  Namespace:  %s\n" "${NAMESPACE}"
...
envsubst < "${REPO_ROOT}/k8s/genie-sim-export-job.yaml" | kubectl apply -n "${NAMESPACE}" -f -

printf "Genie Sim export job submitted: %s\n" "${JOB_NAME}"
