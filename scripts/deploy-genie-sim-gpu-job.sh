...
GPU_NODES=$(kubectl get nodes -l cloud.google.com/gke-accelerator -o name 2>/dev/null || true)
if [[ -z "${GPU_NODES}" ]]; then
  echo "No GPU nodes detected in cluster \"${GKE_CLUSTER}\" (label cloud.google.com/gke-accelerator)." >&2
  echo "Ensure a GPU node pool exists before submitting the job." >&2
  exit 1
fi
