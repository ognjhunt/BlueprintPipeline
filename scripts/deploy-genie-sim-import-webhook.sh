...
JOB_NAME=${JOB_NAME:-"genie-sim-import-webhook-${SCENE_ID:-local}-$(date +%s)"}
...
printf "Deploying Genie Sim import webhook to GKE...\n"
printf "  Project:    %s\n" "${PROJECT_ID}"
printf "  Cluster:    %s\n" "${GKE_CLUSTER}"
printf "  Zone:       %s\n" "${ZONE}"
printf "  Namespace:  %s\n" "${NAMESPACE}"
...
envsubst < "${REPO_ROOT}/k8s/genie-sim-import-webhook.yaml" | kubectl apply -n "${NAMESPACE}" -f -

printf "Genie Sim import webhook applied.\n"
