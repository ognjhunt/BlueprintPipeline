...
JOB_NAME=${JOB_NAME:-"genie-sim-submit-${SCENE_ID:-local}-$(date +%s)"}
...
printf "Deploying Genie Sim submit job to GKE...\n"
printf "  Project:    %s\n" "${PROJECT_ID}"
printf "  Cluster:    %s\n" "${GKE_CLUSTER}"
printf "  Zone:       %s\n" "${ZONE}"
printf "  Namespace:  %s\n" "${NAMESPACE}"
...
envsubst < "${REPO_ROOT}/k8s/genie-sim-submit-job.yaml" | kubectl apply -n "${NAMESPACE}" -f -

printf "Genie Sim submit job submitted: %s\n" "${JOB_NAME}"
