...
- [ ] **Production deployments** set `PIPELINE_ENV=production` and enable `ISAACSIM_REQUIRED=true` + `CUROBO_REQUIRED=true` to enforce runtime checks.
...
Use `PIPELINE_ENV=production` as the canonical production toggle.
