...
4. Generator backend `hybrid_serial` runs (SceneSmith agent loops then SAGE task refinement).
   - Optional: set `text_backend` to `sage` for SAGE-only or `scenesmith` for SceneSmith-only.
