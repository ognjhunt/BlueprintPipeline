#!/usr/bin/env python3
"""
Staging E2E validation for Genie Sim (real gRPC + Isaac Sim runtime).

These tests are gated behind an explicit environment flag so CI stays light.
...
def test_geniesim_staging_export_submit_import(monkeypatch):
    if not _staging_enabled():
        pytest.skip("Set RUN_GENIESIM_STAGING_E2E=1 to run Genie Sim staging tests.")
    ...
    for expected in [
        geniesim_dir / "scene_graph.json",
        geniesim_dir / "asset_index.json",
        geniesim_dir / "task_config.json",
    ]:
        assert expected.is_file(), f"Missing Genie Sim export artifact: {expected}"
