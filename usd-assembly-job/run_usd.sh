...
# 2) Optionally convert GLBs to USD and wire them into the scene.
...
if [[ -n "${GLB_TO_USD_CONVERTER:-}" ]]; then
  ...
  echo "[USD] Running GLB->USD conversion and wiring for scene: ${SCENE_PATH}"
  python convert_glb_to_usd_and_rewire.py \
    --scene "${SCENE_PATH}" \
    --converter "${GLB_TO_USD_CONVERTER}" \
    --root "/mnt/gcs"
else
  echo "[USD] GLB_TO_USD_CONVERTER not set; skipping GLB->USD conversion."
fi
