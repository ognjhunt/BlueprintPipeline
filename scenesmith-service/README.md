...
- `SCENESMITH_SERVICE_MODE=paper_stack` (or `paper`)
  ...
  - Optional controls:
    ...
    - `SCENESMITH_PAPER_OPENAI_WEBSOCKET_BASE_URL` (enable/point OpenAI Responses websocket endpoint for this subprocess)
    - `SCENESMITH_PAPER_OPENAI_USE_WEBSOCKET` (`true|1` to enable websocket mode, if supported by the installed SDK)
