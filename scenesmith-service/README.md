...
- `SCENESMITH_SERVICE_MODE=paper_stack` (or `paper`)
  ...
  - Optional controls:
    ...
    - `SCENESMITH_PAPER_OPENAI_API` (`responses|chat_completions`, defaults to `responses` when unset; forces Agents SDK API for the subprocess)
    ...
    - `SCENESMITH_PAPER_OPENAI_WEBSOCKET_BASE_URL` (enable/point OpenAI Responses websocket endpoint for this subprocess; defaults to `wss://api.openai.com/ws/v1/realtime?provider=openai` when API base is OpenAI)
    - `SCENESMITH_PAPER_OPENAI_USE_WEBSOCKET` (`true|1` to enable websocket mode, if supported by the installed SDK; defaults to `1` when websocket URL is set and no explicit override)
    - `SCENESMITH_PAPER_OPENAI_WEBSOCKET_ENABLED` (legacy alias for `_USE_WEBSOCKET`)
    - `SCENESMITH_PAPER_OPENAI_TRACING_KEY` (override `OPENAI_TRACING_KEY` for the subprocess)
    - `SCENESMITH_PAPER_TIMEOUT_SECONDS` (default `10800`)
    - `SCENESMITH_PAPER_EXISTING_RUN_DIR` (path to an already-completed SceneSmith run dir; skips launching SceneSmith and reuses existing `house_state.json`)
    - `SCENESMITH_PAPER_POST_HOUSE_STATE_GRACE_SECONDS` (default `45`; after `house_state.json` appears, force-exit lingering SceneSmith process if it does not terminate)
    - `SCENESMITH_PAPER_EXIT_PROMPT_MARKERS` (`;;`-delimited list, default includes `Press Ctrl+C to exit`; used to detect interactive-exit hangs)
    - `SCENESMITH_PAPER_WATCH_POLL_SECONDS` (default `2`; watch-loop poll cadence while waiting for SceneSmith)
